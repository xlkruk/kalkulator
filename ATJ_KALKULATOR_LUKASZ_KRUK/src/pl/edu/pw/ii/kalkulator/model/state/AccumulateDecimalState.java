...
public class AccumulateDecimalState implements State {
	...
	public void enterMathOperator(Command operator) {
		if ((calculator.getDisplay().charAt(
				calculator.getDisplay().length() - 1) == ".".charAt(0)))// sprawdzenie
																		// ostaniego
																		// znaku
																		// na
																		// wyswietlaczu
		...
		try {
			...
			calculator.setCurrentOperand(Double.parseDouble(calculator
					.getDisplay()));
			calculator.getPendingOperation().compute();
			if (calculator.hasPendingOperation()) {
				calculator.setDisplay(new BigDecimal(calculator.getResult())
						.stripTrailingZeros().toPlainString());// formatowanie}
	...
	public void enterSignChange() {
		calculator.setDisplay(new BigDecimal((-1)
				* Double.parseDouble(calculator.getDisplay()))
				.stripTrailingZeros().toPlainString());// formatowanie
		calculator.notifyDisplayObservers();
	}
